...
public class movieAdapter extends BaseAdapter {
    ...
    public View getView(int position, View view, ViewGroup viewGroup) {
        ...
        //绑定数据
        Movie newItem = this.list.get(position);
        String imagePath = newItem.getImg();
        ...
        tv1.setText("影片名："+list.get(position).gettCn());
        tv2.setText("主演一："+list.get(position).getaN1());
        tv3.setText("主演二："+list.get(position).getaN2());
        tv4.setText("导演："+list.get(position).getdN());
        tv5.setText("影片描述："+list.get(position).getCommonSpecial());
        //图片加载??????????
        return temp;
    }
}
